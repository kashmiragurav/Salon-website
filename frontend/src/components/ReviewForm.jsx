...
function reviewErrorMessage(error) {
  if (error?.code === "permission-denied") return "Your review could not be submitted. Please sign in and try again.";
  ...
  if (error?.code?.includes("network") || error?.code === "unavailable") return "Unable to connect right now. Please check your internet connection and try again.";
...
export default function ReviewForm() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      console.error("Review submission failed:", { code: error?.code, message: error?.message });
  ...
  return <form className="form-panel review-form" onSubmit={submit} noValidate>
    ...
    <button className="button" disabled={state.loading || state.sent}>{state.loading ? "Submitting..." : "Submit review"}</button>
  </form>;
}
