...
import { auth, db, getAuthUser } from "../firebase/config";
...
export const createBooking = async (booking) => {
  ...
  if (!user?.uid) throw new Error("Please sign in before booking an appointment.");
...
export const cancelBooking = async (bookingId) => {
  const user = await getAuthUser();
  if (!user?.uid) throw new Error("Please sign in to cancel an appointment.");
...
export const subscribeToMyBookings = (userId, onData, onError) => onSnapshot(
...
export const getMyBookings = async (userId) => {
  const user = await getAuthUser();
  if (!user?.uid || user.uid !== userId) throw new Error("A valid signed-in client is required.");
...
export const getBookingById = async (bookingId) => {
  const user = await getAuthUser();
  if (!user?.uid) throw new Error("A signed-in client is required.");
  const snapshot = await getDoc(doc(db, "bookings", bookingId));
  if (!snapshot.exists()) return null;
  const booking = { id: snapshot.id, ...snapshot.data() };
  return booking.userId === user.uid ? booking : null;
};
