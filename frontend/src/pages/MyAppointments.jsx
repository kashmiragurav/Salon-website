...
const isPast = (booking) => booking.status !== "Cancelled" && appointmentDate(booking)?.getTime() < Date.now();
...
export default function MyAppointments() {
  ...
  useEffect(() => { getMyBookings(user.uid).then((bookings) => setState({ bookings, loading: false, error: "" })).catch((error) => { console.error(error); setState({ bookings: [], loading: false, error: "Unable to load your appointments. Please try again." }); }); }, [user.uid]);
  const visible = useMemo(() => state.bookings.filter((booking) => tab === "All" || (tab === "Upcoming" && isUpcoming(booking)) || (tab === "Past" && isPast(booking)) || (tab === "Cancelled" && booking.status === "Cancelled")), [state.bookings, tab]);
