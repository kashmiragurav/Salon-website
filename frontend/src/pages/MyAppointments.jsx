...
export default function MyAppointments() {
  ...
  const setTab = (next) => setSearchParams(next === "All" ? {} : { tab: next }, { replace: true });
  ...
  useEffect(() => {
    const unsubscribe = subscribeToMyBookings(
      user.uid,
      (bookings) => setState({ bookings, loading: false, error: "" }),
      (error) => {
        console.error("My appointments failed to load:", error);
        setState({ bookings: [], loading: false, error: "Unable to load your appointments. Please try again." });
      },
    );
    return unsubscribe;
  }, [user.uid]);
  ...
  const cancel = async (booking) => {
    if (!canCancel(booking) || !window.confirm("Are you sure you want to cancel this appointment?")) return;
    try {
      await cancelBooking(booking.id);
      setState((current) => ({ ...current, bookings: current.bookings.map((item) => item.id === booking.id ? { ...item, status: "Cancelled" } : item) }));
      setSelected(null);
      setNotice("Appointment cancelled successfully.");
    } catch (error) {
      console.error(error);
      setNotice("Unable to cancel this appointment. Please try again.");
    }
  };
  ...
  return (
    <section className="section page-intro appointments-page">
      ...
      {notice && <p className={notice.startsWith("Appointment") ? "success-message" : "error-message"} role="status">{notice}</p>}
      {state.loading && <LoadingState label="Loading your appointments..." />}
      ...
      {!state.loading && !state.error && visible.length > 0 && (
        <div className="client-appointments-grid">
          {visible.map((booking) => (
            <article className="client-appointment-card" key={booking.id}>
              <span className="kicker">{booking.status}</span>
              <h2>{booking.serviceSelected}</h2>
              <p><CalendarDays size={15} />{formatDate(booking)} · {booking.preferredTime || "Time unavailable"}</p>
              <small>Booking ID: {booking.id}</small>
              <div>
                <button type="button" className="button-secondary" onClick={() => setSelected(booking)}><Eye size={15} /> View details</button>
                {canCancel(booking) && <button type="button" className="button-danger" onClick={() => cancel(booking)}>Cancel</button>}
              </div>
            </article>
          ))}
        </div>
      )}
      {selected && <AppointmentDetailsModal booking={selected} onClose={() => setSelected(null)} onCancel={canCancel(selected) ? () => cancel(selected) : undefined} />}
    </section>
  );
}
