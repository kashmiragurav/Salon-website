...
import { subscribeToApprovedTestimonials } from "../services/testimonialService";
import { subscribeToActiveContent } from "../services/contentService";
...
function Home({ settings }) {
  ...
  return <>{settings.error && <ErrorState message="We could not load salon information. Please try again later." />}
    ...
    {salon?.whatsappNumber && <div className="floating-actions"><a className="floating-whatsapp" href={`https://wa.me/${salon.whatsappNumber.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" aria-label="Chat with us on WhatsApp"><MessageCircle size={21} /></a></div>}
  </>;
}

const loadServices = () => subscribeToActiveServices;
const loadTestimonials = () => subscribeToApprovedTestimonials;
const loadOffers = () => (onData, onError) => subscribeToActiveContent("offers", onData, onError);
const loadPackages = () => (onData, onError) => subscribeToActiveContent("packages", onData, onError);
const loadTeam = () => (onData, onError) => subscribeToActiveContent("team", onData, onError);
const loadBeforeAfter = () => (onData, onError) => subscribeToActiveContent("beforeAfter", onData, onError);
const loadFaqs = () => (onData, onError) => subscribeToActiveContent("faqs", onData, onError);
